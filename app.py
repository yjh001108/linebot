...
def add_record(user_id, item, category, amount, note=""):
    ...
    c = conn.cursor()
    c.execute(
        'INSERT INTO records (user_id, record_date, item, category, amount, note, created_at) VALUES (?,?,?,?,?,?,?)',
...
def export_csv(user_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT record_date, item, category, amount, note FROM records WHERE user_id=? ORDER BY id DESC", (user_id,))
    rows = c.fetchall()
    conn.close()

    with open(out_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["日期","品項","分類","金額","備註"])  # 中文欄位名稱
        for r in rows:
            writer.writerow([r[0], r[1], r[2], int(r[3]), r[4] or ""])  # 金額整數化
    return out_path
...
def handle_message(event):
    ...
    try:
        ...
        if len(parts) >= 3 and parts[2].isdigit():
            item = parts[0]
            category = parts[1]
            amount = int(parts[2])  # 金額整數化
            note = " ".join(parts[3:]) if len(parts) > 3 else ""
            add_record(user_id, item, category, amount, note)
            reply = f"✅ 已記帳：{date.today()} | {item} | {category} | {amount} | {note}"
        elif text in ["清單"]:
            rows = query_records(user_id, "recent")
            reply = "\n".join([f"{r[0]} | {r[1]} | {r[2]} | {int(r[3])} | {r[4] or ''}" for r in rows]) or "沒有紀錄"
        elif text.isdigit() and len(text)==4:  # ex: 0902
            rows = query_records(user_id, "date", text)
            reply = "\n".join([f"{r[0]} | {r[1]} | {r[2]} | {int(r[3])} | {r[4] or ''}" for r in rows]) or f"{text} 沒有紀錄"
        elif text in ["今日"]:
            rows = query_records(user_id, "today")
            reply = "\n".join([f"{r[0]} | {r[1]} | {r[2]} | {int(r[3])} | {r[4] or ''}" for r in rows]) or "今天沒有紀錄"
        elif text in ["本月"]:
            rows = query_records(user_id, "month")
            reply = "\n".join([f"{r[0]} | {r[1]} | {r[2]} | {int(r[3])} | {r[4] or ''}" for r in rows]) or "本月沒有紀錄"
        elif text in ["儲存"]:
